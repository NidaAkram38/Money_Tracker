...
function setLockMode(mode) {
  ...
  document.getElementById('lock-btn').textContent       = isSignup ? 'Create Account ✨' : 'Let me in! ✨';
  document.getElementById('lock-switch').textContent    = isSignup ? 'Already have account? Login →' : 'New here? Create account →';
  document.getElementById('lock-sub').textContent       = isSignup ? 'Set up your PIN 🔒' : 'Your cute little money diary 🌸';
  document.getElementById('signup-fields').style.display    = isSignup ? 'block' : 'none';
  document.getElementById('pin-confirm-wrap').style.display = isSignup ? 'block' : 'none';
}
...
function getAllCats() {
  ...
  const custom = (data.customCats || []).map(c => ({
    ...
    label: c.label, icon: c.icon || '🏷️', bg: '#f2eeff',
  }));
...
function populateMonthSelect() {
  ...
  keys.forEach(k => {
    ...
    o.value = k; o.textContent = monthShort(k);
...
function switchMonth() {
  ...
  document.getElementById('greet-month').textContent = monthLabel(currentMonth);
  // Show existing budget in the input
  const data = loadData();
  const md   = data[currentMonth] || {};
  document.getElementById('budget-input').value = md.budget || '';
  document.getElementById('bsc-hint').textContent = md.budget ? 'Current: Rs. ' + md.budget.toLocaleString() : '';
  renderDashboard();
}
...
function saveBudget() {
  ...
  if (!raw || raw.toString().trim() === '') {
    document.getElementById('bsc-hint').textContent = '⚠️ Please enter an amount first.';
    document.getElementById('bsc-hint').style.color = '#ff5a7a';
    return;
  }
  if (isNaN(val) || val <= 0) {
    document.getElementById('bsc-hint').textContent = '⚠️ Enter a number greater than 0.';
    document.getElementById('bsc-hint').style.color = '#ff5a7a';
    return;
  }
  ...
  document.getElementById('bsc-hint').style.color = '#34c77b';
  document.getElementById('bsc-hint').textContent = '✅ Budget saved! Rs. ' + val.toLocaleString();
  renderDashboard();
}
...
function renderDashboard() {
  ...
  // Show existing budget in input box if set
  if (budget > 0) {
    document.getElementById('budget-input').value = budget;
    document.getElementById('bsc-hint').textContent = '';
  } else {
  ...
  fillEl.style.background = pct > 90 ? '#ff5a7a' : pct > 70 ? '#ffca3a' : '#ffe066';

  if (left >= 0) {
    leftEl.textContent = 'Rs. ' + left.toLocaleString();
    leftEl.className   = 'hero-left-val';
    labelEl.textContent = 'Remaining 🎉';
  } else {
    leftEl.textContent = '− Rs. ' + Math.abs(left).toLocaleString();
    leftEl.className   = 'hero-left-val over';
    labelEl.textContent = 'Over Budget ⚠️';
  }
  ...
  if (sorted.length === 0) {
    listEl.innerHTML = `<div class="empty-state"><span class="empty-state-icon">🌸</span>No expenses yet! Tap + to add one.</div>`;
    return;
  }
...
function renderCategoryChips() {
  ...
  wrap.innerHTML = getAllCats().map(cat => `
...
function addExpense() {
  ...
  const expense  = { id: Date.now(), cat: selectedCat, catLabel: cat.label, catIcon: cat.icon, amount, date, note, place };
...
function renderHistory() {
  ...
  el.innerHTML = valid.map(key => {
    ...
    const totals = {};
    expenses.forEach(e => {
      if (!totals[e.cat]) totals[e.cat] = { total: 0, cat: e.cat };
      totals[e.cat].total += e.amount;
    });
    const maxAmt = Math.max(...Object.values(totals).map(t => t.total), 1);

    const breakdown = Object.entries(totals).sort((a,b)=>b[1].total-a[1].total).map(([catId, info]) => {
      const cat = getCat(catId);
      return `<div class="cat-row">
        <span class="cat-row-icon">${cat.icon}</span>
        <span class="cat-row-name">${cat.label}</span>
        <div class="cat-row-bar"><div class="cat-row-fill" style="width:${Math.round((info.total/maxAmt)*100)}%"></div></div>
        <span class="cat-row-amt">Rs.${info.total.toLocaleString()}</span>
      </div>`;
    }).join('');

    const expRows = [...expenses].sort((a,b)=>new Date(b.date)-new Date(a.date)).map(e => {
      const cat  = getCat(e.cat);
      const meta = [e.place, e.note, formatDate(e.date)].filter(Boolean).join(' · ');
      return `<div class="hist-expense-row">
        <div class="hist-exp-left">
          <span style="font-size:18px;">${cat.icon}</span>
          <div><div class="hist-exp-cat">${cat.label}</div><div class="hist-exp-meta">${meta}</div></div>
        </div>
        <div class="hist-exp-amt">−Rs.${e.amount.toLocaleString()}</div>
      </div>`;
    }).join('');

    const lc  = left >= 0 ? 'left' : 'over';
    const ll  = left >= 0 ? 'Left' : 'Over';
    return `<div class="month-card">
      <div class="month-header" onclick="toggleHistory('${key}')">
        <div class="month-name">${monthLabel(key)}</div>
        <div class="month-chips">
          <div class="chip-stat"><span class="cs-label">Budget</span><span class="cs-val">Rs.${budget.toLocaleString()}</span></div>
          <div class="chip-stat spent"><span class="cs-label">Spent</span><span class="cs-val">Rs.${spent.toLocaleString()}</span></div>
          <div class="chip-stat ${lc}"><span class="cs-label">${ll}</span><span class="cs-val">Rs.${Math.abs(left).toLocaleString()}</span></div>
        </div>
      </div>
      <div class="month-body" id="hist-${key}">
        ${expenses.length === 0 ? '<p style="color:var(--text3);font-size:13px;padding-top:12px;">No expenses.</p>'
...
function addCustomCat() {
  ...
  if (data.customCats.find(c => c.label.toLowerCase() === name.toLowerCase())) { alert('Already exists!'); return; }
  data.customCats.push({ label: name, icon: icon || '🏷️' });
  saveData(data);
  document.getElementById('new-cat-name').value = '';
  document.getElementById('new-cat-icon').value = '';
  renderCustomCats(); renderCategoryChips();
}
...
function renderCustomCats() {
  ...
  if (cats.length === 0) { el.innerHTML = '<p style="font-size:13px;color:var(--text3);font-weight:600;margin-top:10px;">No custom categories yet 🌸</p>'; return; }
  el.innerHTML = cats.map(c => `<div class="custom-cat-tag"><span>${c.icon}</span> ${c.label}<button onclick="removeCustomCat('${c.label}')">✕</button></div>`).join('');
}
...
// ── CLEAR MONTH ──
function clearMonth() {
  if (!confirm('Delete ALL data for ' + monthLabel(currentMonth) + '?\nThis cannot be undone!')) return;
  const data = loadData();
  delete data[currentMonth];
  saveData(data);
  populateMonthSelect(); renderDashboard(); renderHistory();
}
